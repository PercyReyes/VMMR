...
for i = 1:trnSize
    ...
    svm_trainingSet(i).keypoints = detectSURFFeatures(I);
    
    % extract SURF features from image
    training_features(i,:) = sum(extractFeatures(svm_trainingSet(i).images, svm_trainingSet(i).keypoints,'SURF',128));
...
for i = 1:tstSize
    ...
    % detect SURF features within each image
    svm_testSet(i).keypoints = detectSURFFeatures(I);
    
    % extract SURF features from image
    test_features(i,:) = sum(extractFeatures(svm_testSet(i).images, svm_testSet(i).keypoints ,'SURF', 128));
...
oofLabels = kfoldPredict(cv_svm_model);
